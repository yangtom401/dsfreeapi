...
`model_types` in `[deepseek]` config (default: `["default", "expert", "vision"]`) maps to OpenAI model ID `deepseek-{type}` (e.g., `deepseek-default`, `deepseek-expert`, `deepseek-vision`). Anthropic compat uses the same IDs.
...
- `cargo test` compiles examples too (slower); use `cargo test --lib` for faster iteration on library-only changes

### Lint Configuration (Cargo.toml)

Strict `deny`-level lints are enforced — code will not compile with warnings:
- `unsafe_code = "deny"` — no unsafe blocks allowed
- Clippy: `all = "deny"` plus 30+ additional `deny` lints (see `[lints.clippy]` in `Cargo.toml`)
- `future_not_send = "deny"` — all futures must be `Send` (required for tokio spawning)
- `wildcard_imports = "deny"` — no `use foo::*`
- `tests_outside_test_module = "deny"` — all tests must be inside `#[cfg(test)] mod`
...
| `DS_DATA_DIR` | `.` (cwd) | Data directory for `logs/runtime.log` and `stats.json` |
...
# Enable pre-commit hook (runs: just check-web, just check, cargo test)
git config core.hooksPath .githooks

# One-pass check (fmt + check + clippy + audit + outdated + machete)
just check

# Frontend check (bun install --frozen-lockfile + typecheck + build + lint)
just check-web
...
just e2e-oversized # Oversized: 长上下文回退方案测试（expert 分块 + default/vision 文件上传）
...
cargo outdated --exit-code 1 --root-deps-only # requires: cargo install cargo-outdated
